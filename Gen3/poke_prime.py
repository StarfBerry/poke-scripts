...
from RNG import LCRNG, LCRNGR, lcrng_recover_pid_seeds
                            ...
                            for o in primes:
                                ...
                                for h in primes[1:]:
                                    ...
                                    if is_prime(pid) and is_prime(pid >> 16) and is_prime(pid & 0xffff) and is_prime(nature := pid % 25):
                                        for seed in lcrng_recover_pid_seeds(pid):
                                            for method in methods:
                                                if prime_ivs(ivs := pid_seed_to_ivs(seed, method)):
                                                    print(fmt.format(pid, format_ivs(ivs), get_nature(pid), nature, get_hp_type(ivs), get_hp_damage(ivs), method, seed))
